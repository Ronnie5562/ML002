...
data = (
    resp.read()
)  # Note: once you read the response, python closes the connection and the { resp } becomes an empty string.
html = data.decode(
    "UTF-8"
)  # we have to decode it, because the data is actuallt in bytes;
print(html)
